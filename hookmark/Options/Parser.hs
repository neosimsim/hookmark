...
parseOptions = eP $ info
  (optionsParser <**> helper)
  (fullDesc <> progDesc programDescription <> header
  ...
  Version
    ...
    <|> hsubparser
          ...
          <> command
               ...
               (info
                 gitParser
                 (progDesc "Invoke git within hookmark directory" <> footerDoc
                   (   Just
...
wrapper pinfo args = do
  ...
  case cmd of
    MoveBookmark { moveSources = sources } -> if null sources
      then Failure $ parserFailure
        defaultPrefs
        pinfo
